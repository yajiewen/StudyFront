<template>
  <div id="identityv" class="columns">
    <div class="column is-6 is-offset-3">
      <p class="font2">待身份审核数: {{identinfo2.num}}</p>
      ...
      <div class="inbox-messages" id="inbox-messages">
        <table class="table font2">
          ...
          <tr v-for="(info,index) in identinfo2.infolist.slice(slicestart,sliceend)">
            <td>{{info.usr_email}}</td>
            <td><a :href="info.usr_identity_imgurl1" target="_blank"><img :src="info.usr_identity_imgurl1" alt=""></a></td>
            <td><a :href="info.usr_identity_imgurl2" target="_blank"><img :src="info.usr_identity_imgurl2" alt=""></a></td>
...
export default {
  ...
  data(){
    return{
      identinfo2:this.identinfo,  //为了不删除后请求刷新增加一个副本
      pageindex:0,   //页码
  ...
  computed:{
    ...
    //切片结束位置
    sliceend(){
      if (this.slicestart + this.eachpageitemnum > this.identinfo2.num){
        return this.identinfo2.num
      }else{
    ...
    //页数
    pagenum(){
      return Math.ceil(this.identinfo2.num/this.eachpageitemnum)
    }
  },
  methods:{
    prepage(){
    ...
    upload_result(emial,index,status){
      axios({
        ...
        method:'post',
        url:'https://127.0.0.1:8081/backstage/iverify/',
        data:{
          uemail:emial,
          status:status,
        }
        ...
        if(res.data.is_login=='yes'){
          if(res.data.is_deal == 'yes'){
            ...
            //删除元素使它不显示
            this.identinfo2.infolist.splice(itemindex,1)
            this.identinfo2.num -=1
          }
        }else{
          alert('请重新登录')
        }
